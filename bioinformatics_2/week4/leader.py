...
def spectrum_score(peptide, exp_spec):
    ...
    pep_spec = list(spectrum(peptide))
    exp_spec = list(exp_spec)
...
if __name__ == "__main__":
    ...
    with open(
        "C:/Users/Paul/src/bioinformatics/bioinformatics_2/week4/dataset_102_8(4).txt"
    ) as input_data:
        n, spec = [
            int(line.strip()) if i == 0 else map(int, line.strip().split())
    ...
    seq = filter(
        lambda L: L[0] != -1,
        [[spectrum_score(peptide, spec), peptide] for peptide in append_protein(weight.keys())],
    )
    ...
    while seq != []:
        ...
        seq = filter(
            lambda L: L[0] != -1,
            [[spectrum_score(peptide, spec), peptide] for peptide in append_protein(leaders)],
        )
    ...
    with open(
        "C:/Users/Paul/src/bioinformatics/bioinformatics_2/week4/output.txt", "w"
    ) as output_data:
